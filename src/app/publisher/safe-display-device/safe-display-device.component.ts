...
import { SafeRevenueDisplayDeviceService } from './safe-display-device.service';
declare var $: any;
...
@Component({
  selector: 'app-network-revenue-display-device',
  templateUrl: './safe-display-device.component.html',
  styleUrls: ['./safe-display-device.component.scss'],
  // Tooltip start
...
export class SafeRevenueDisplayDeviceComponent implements OnInit {
  // Tooltip start
  ...
  constructor(private _displayDeviceService: SafeRevenueDisplayDeviceService, private datePipe: DatePipe, public dialog: MatDialog) {
...
Highcharts.setOptions({
        ...
        events: {
          drilldown: function (e) {
            ...
            if (e.seriesOptions.name.search("By") > -1) {
              ...
              const level2fil = level2.filter(x => x.level1value === e.seriesOptions.name);
  ...
  ngOnInit() {
    ...
    //api
    this._displayDeviceService.getSafeDisplayDevice(localStorage.getItem('startdate'), localStorage.getItem('enddate'), localStorage.getItem('uniq_id'), localStorage.getItem('child_net_code'), localStorage.getItem('range'))
      .subscribe(dataResult => {

        if (dataResult['status_code'] == '200') {
          this.graphrootdata = dataResult['data']["level1"];
          this.tablerootdata = dataResult['data']["device_table_data"];
          this.tablerootdatainner = dataResult['data']["device_innertable_data"];
          ...
          this.lvl2data = dataResult['data']['device_table_data_lvl1'];
  ...
  findDetails(data) {
    return this.tablerootdatainner.filter(x => x.whoseData === data.device);
  }

  downloadsafedevice() {

    //api
    this._displayDeviceService.downSafeDisplayDevice(localStorage.getItem('startdate'), localStorage.getItem('enddate'), localStorage.getItem('uniq_id'), localStorage.getItem('child_net_code'), localStorage.getItem('range'))
      .subscribe(dataResult => {
