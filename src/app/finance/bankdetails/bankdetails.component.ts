...
export class BankDetailsComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private location: Location, private _bankdetailService: BankdetailService, public dialog: MatDialog, private http: HttpClient, private router: Router,activatedRouter: ActivatedRoute) {
    this.partner_id = activatedRouter.snapshot.paramMap.get('uniqId');
  ...
  ngOnInit() {
    this.bankdetailform = this.fb.group({
      ...
      org_type: new FormControl({value: null, disabled: true}),
      org_country: new FormControl({value: null, disabled: true}),
      org_address: new FormControl({value: null, disabled: true}),
      org_city: new FormControl({value: null, disabled: true}),
      org_state: new FormControl({value: null, disabled: true}),
      org_postelcode: new FormControl({value: null, disabled: true}),
      acc_holder_name: new FormControl({value: null, disabled: true}),
      bank_name: new FormControl({value: null, disabled: true}),
      account_number: new FormControl({value: null, disabled: true}),
      bank_swift_code: new FormControl({value: null, disabled: true}),
      bank_ifsc: new FormControl({value: null, disabled: true}),
      bank_address: new FormControl({value: null, disabled: true}),
      bank_city: new FormControl({value: null, disabled: true}),
      bank_state: new FormControl({value: null, disabled: true}),
      bank_postalcode: new FormControl({value: null, disabled: true}),
      bank_acctype: new FormControl({value: null, disabled: true}),
      GST: new FormControl({value: null, disabled: true}),
      GST_address: new FormControl({value: null, disabled: true}),
      GST_city: new FormControl({value: null, disabled: true}),
      GST_state: new FormControl({value: null, disabled: true}),
      GST_postalcode: new FormControl({value: null, disabled: true}),
      GST_num: new FormControl({value: null, disabled: true}),
      PAN_num: new FormControl({value: null, disabled: true}),
      GST_file: new FormControl({value: null, disabled: true}),
      pan_file: new FormControl({value: null, disabled: true}),
      incorp_file: new FormControl({value: null, disabled: true}),
      aadhaar_file: new FormControl({value: null, disabled: true}),
      cancelcheck_file: new FormControl({value: null, disabled: true}),
      status_bank: [null, Validators.compose([Validators.required,Validators.minLength(1)])],
    });
    //country api code
    ...
    this._bankdetailService.getbankDetail(this.partner_id).subscribe((response) => {
      if (response['status_code'] == '200') {
        ...
        this.pub_uniq_id_m = response['resultdata'][0]['uniq_id']; 
  ...
  onPercentChange(event) {
    ...
    if (org_country == 99) {
      ...
      this.bankdetailform.get('GST_postalcode').setValidators([Validators.required, Validators.pattern("^[0-9]*$")]);
  ...
  //   });
  // }
  bankStatusChng(pub_uniq_id_m:any){
    let currentUrl = this.router.url;
    if(pub_uniq_id_m==this.partner_id){
      console.log("echeck=>"+pub_uniq_id_m);
      console.log(this.bankdetailform);
      this.isSpinnerVisible = true;
      const fData: any = new FormData;
      fData.append("uniq_id", localStorage.getItem('uniq_id'));
      fData.append("partner_uniq_id", this.partner_id);
      fData.append("status_bank", this.bankdetailform.value.status_bank);
      var t = localStorage.getItem('access_token');
      var headers_object = new HttpHeaders().set("Authorization", "Bearer " + t);
      const httpOptions = {
        headers: headers_object
      };
      this.http.post(AppSettings.API_STARTPOINT+'/assets/api/admin/finance/update_bankstatus.php', fData, httpOptions).subscribe(data => {
